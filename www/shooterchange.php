...
// first run query
    ...
    $resultMannschaften2 = mysqli_query($dbconn, "select MannschaftsID,Verein from `dorfpokal_mannschaft` order by Verein ");
    mysqli_close($dbconn);
...
<FORM NAME="ergebnis" action="shooterchange_commit.php" method="post">
    ...
    <tr><td>Neue Mannschaft</td>
    <td><SELECT id="newteam" name="newteam">';
    while($mannschaft = mysqli_fetch_array($resultMannschaften2, MYSQLI_ASSOC))
    {
	     print ("      <OPTION VALUE=".$mannschaft['MannschaftsID'].">".$mannschaft['Verein']."\n");
    }
    echo '</SELECT>
